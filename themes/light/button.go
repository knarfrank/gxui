...
func CreateButton(theme *Theme) gxui.Button {
	...
	b.Init(b, theme)
	b.theme = theme
	b.SetPadding(math.Spacing{L: 8, T: 5, R: 8, B: 5})
...
func (b *Button) Paint(c gxui.Canvas) {
	...
	c.DrawRoundedRect(r, 4, 4, 4, 4, gxui.TransparentPen, brush)

	b.PaintChildren.Paint(c)

	c.DrawRoundedRect(r, 4, 4, 4, 4, pen, gxui.TransparentBrush)

	if b.IsChecked() {
		pen = b.theme.HighlightStyle.Pen
		brush = b.theme.HighlightStyle.Brush
		c.DrawRoundedRect(r, 4.0, 4.0, 4.0, 4.0, pen, brush)
	}
